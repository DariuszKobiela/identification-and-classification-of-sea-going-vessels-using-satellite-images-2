...
def list_asf_files(show_logs=False):
    asf_files_list = os.listdir(PROCESSED_ASF_DATA_PATH)
...
def calculate_asf_image_features(show_logs=False):
    ...
    for asf_file in tqdm(asf_files_list, desc="Calculating ASF image features"):
        ...
        ds = gdal.Open(PROCESSED_ASF_DATA_PATH + '/' + asf_file)
...
def create_ais_df(show_logs=False):
    ais_files_list = os.listdir(RAW_AIS_CSV_DATA_PATH)
...
def restrict_data_by_given_conditions(full_data_df, save=True, show_logs=False):
    ...
    for index, row in tqdm(full_data_df.iterrows(), total=full_data_df.shape[0], desc="Restricting AIS data by given conditions"):
        ...
        if save:
            ...
            target_area_and_time.to_csv(PROCESSED_AIS_DATA_PATH + '/' + saving_name)
...
def interpolate_ais_data(full_data_df, save=True, show_logs=False):
    ...
    for index, row in tqdm(full_data_df.iterrows(), total=full_data_df.shape[0], desc="Interpolating AIS data"):
        ...
        file_path = PROCESSED_AIS_DATA_PATH + '/' + str(row['processed_ais_file'])
...
asf_files_list = list_asf_files(show_logs=False)
asf_df = calculate_asf_image_features(show_logs=False)
ais_df = create_ais_df(show_logs=False)
full_data_df = create_full_data_df(asf_df, ais_df, save=True, show_logs=False)
full_data_df = restrict_data_by_given_conditions(full_data_df, show_logs=False)
df_after_lin_reg = interpolate_ais_data(full_data_df, save=True, show_logs=False)
print(df_after_lin_reg)
